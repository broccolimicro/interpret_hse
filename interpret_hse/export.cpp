...
{
	string result = "unstable assignment T" + to_string(i.effect.index) + "." + to_string(i.effect.term) + ":";
	if (g.transitions[i.effect.index].behavior == hse::transition::active)
		result += export_internal_choice(g.transitions[i.effect.index].action[i.effect.term], v).to_string();
	else
		result += "[" + export_disjunction(g.transitions[i.effect.index].action[i.effect.term], v).to_string() + "]";

	result += " cause: {";

	for (int j = 0; j < (int)i.cause.size(); j++)
	{
		if (j != 0)
			result += "; ";

		result += "T" + to_string(i.cause[j].index) + "." + to_string(i.cause[j].term) + ":";

		if (g.transitions[i.cause[j].index].behavior == hse::transition::active)
			result += export_internal_choice(g.transitions[i.cause[j].index].action[i.cause[j].term], v).to_string();
		else
			result += "[" + export_disjunction(g.transitions[i.cause[j].index].action[i.cause[j].term], v).to_string() + "]";
	}
	result += "}";
	return result;
}
...
{
	string result = "interfering assignments T" + to_string(i.first.index) + "." + to_string(i.first.term) + ":";
	if (g.transitions[i.first.index].behavior == hse::transition::active)
		result += export_internal_choice(g.transitions[i.first.index].action[i.first.term], v).to_string();
	else
		result += "[" + export_disjunction(g.transitions[i.first.index].action[i.first.term], v).to_string() + "]";
	result += " and T" + to_string(i.second.index) + "." + to_string(i.second.term) + ":";
	if (g.transitions[i.second.index].behavior == hse::transition::active)
		result += export_internal_choice(g.transitions[i.second.index].action[i.second.term], v).to_string();
	else
		result += "[" + export_disjunction(g.transitions[i.second.index].action[i.second.term], v).to_string() + "]";
	return result;
}
...
string export_deadlock(boolean::variable_set &v, hse::deadlock d)
{
	...
	for (int i = 0; i < (int)d.tokens.size(); i++)
	{
		if (i != 0)
			result += "  ";
		result += export_token(v, d.tokens[i]);
	}
	result += "}";
