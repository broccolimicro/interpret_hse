...
hse::graph import_hse(const parse_cog::composition &syntax, ucs::variable_set &variables, boolean::cover &covered, bool &hasRepeat, int default_id, tokenizer *tokens, bool auto_define) {
	...
	for (int i = 0; i < (int)syntax.branches.size(); i++)
	{
		if (syntax.branches[i].sub != nullptr and syntax.branches[i].sub->valid) {
			...
			result.merge(composition, import_hse(*syntax.branches[i].sub, variables, subcovered, subRepeat, default_id, tokens, auto_define));
			...
			if (syntax.branches[i].ctrl->guard.valid) {
				subcovered = import_cover(syntax.branches[i].ctrl->guard, variables, default_id, tokens, auto_define);
			}
			if (composition == hse::choice) {
				covered |= subcovered;
			} else if (composition == hse::parallel) {
...
hse::graph import_hse(const parse_cog::control &syntax, ucs::variable_set &variables, int default_id, tokenizer *tokens, bool auto_define) {
	...
	if (syntax.guard.valid and import_cover(syntax.guard, variables, default_id, tokens, auto_define) != 1) {
		result.merge(hse::sequence, import_hse(syntax.guard, variables, syntax.kind == "assume", default_id, tokens, auto_define));
	}
	if (syntax.action.valid) {
		boolean::cover covered;
		bool hasRepeat = false;
		result.merge(hse::sequence, import_hse(syntax.action, variables, covered, hasRepeat, default_id, tokens, auto_define));
	}
