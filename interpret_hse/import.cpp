		...
		for (int i = 0; i < (int)syntax.actions.size(); i++)
		{
			if (syntax.actions[i]->valid)
			{
				...
				if (syntax.actions[i]->is_a<parse_hse::parallel>())
					...
					m = import_graph(tokens, *(parse_hse::sequence*)syntax.actions[i], variables, g, m, auto_define);
					...
					m = import_graph(tokens, *(parse_boolean::assignment*)syntax.actions[i], variables, g, m, auto_define);
			}
		}
		...
		for (int i = 0; i < (int)syntax.branches.size(); i++)
		{
			if (syntax.branches[i]->valid)
			{
				...
				if (syntax.branches[i]->is_a<parse_hse::parallel>())
					temp = import_graph(tokens, *(parse_hse::parallel*)syntax.branches[i], variables, g, temp, auto_define);
				else if (syntax.branches[i]->is_a<parse_hse::sequence>())
					temp = import_graph(tokens, *(parse_hse::sequence*)syntax.branches[i], variables, g, temp, auto_define);
				else if (syntax.branches[i]->is_a<parse_hse::condition>())
					temp = import_graph(tokens, *(parse_hse::condition*)syntax.branches[i], variables, g, temp, auto_define);
				else if (syntax.branches[i]->is_a<parse_hse::loop>())
					temp = import_graph(tokens, *(parse_hse::loop*)syntax.branches[i], variables, g, temp, auto_define);
				else if (syntax.branches[i]->is_a<parse_boolean::assignment>())
					temp = import_graph(tokens, *(parse_boolean::assignment*)syntax.branches[i], variables, g, temp, auto_define);
